//! `file_sha256` hashes every block of a PNG except its XMP.
//!
...
use pan::xmp::{file_sha256, write_packet_into_png_bytes};
...
fn three_different_files_hash_three_different_ways() {
    let rgb8 = file_sha256(&fixture("rgb8.png")).unwrap();
    let rgba8 = file_sha256(&fixture("rgba8.png")).unwrap();
    let rgb16 = file_sha256(&fixture("rgb16.png")).unwrap();
...
fn writing_xmp_does_not_move_the_hash() {
    ...
    let before = file_sha256(&png).unwrap();
    ...
    assert_eq!(file_sha256(&once).unwrap(), before, "adding XMP moved it");
    assert_eq!(
        file_sha256(&twice).unwrap(),
        before,
...
fn the_same_bytes_hash_the_same_every_time() {
    assert_eq!(
        file_sha256(&fixture("rgb8.png")).unwrap(),
        file_sha256(&fixture("rgb8.png")).unwrap()
    );
}

...
fn one_depth_run_lands_map_sidecar_record_graph_and_xmp() {
    ...
    assert!(
        ...
        "image is pending for depth before the run"
    );
    ...
    assert!(
        ...
        "the bytes live in the PNG, not the sidecar"
    );
    ...
    let refs = facts
        .iter()
        .find(|(p, _)| p == &format!("{}{REF_LOCAL}", pan::PAN_NS))
        .map(|(_, v)| v.clone())
        .unwrap_or_default();
    ...
    assert_eq!(
        rows.len(),
        1,
        "one Depth record reachable through the reference: {rows:?}"
    );
    assert_eq!(
        rows[0],
        vec![
    ...
    assert!(
        xmp.contains(&format!("<pan:path>{rel}</pan:path>")),
        "{xmp}"
    );
    assert!(
        !xmp.contains("<pan:count>"),
        "a depth reference carries no count: {xmp}"
    );
    ...
    assert!(
        !pending.iter().any(|p| p.id == id),
        "image retired from the depth queue"
    );
    ...
    assert!(
        st.enrichment
            .iter()
            .any(|(l, ms)| l == REF_LOCAL && ms.iter().any(|m| m == model)),
        "{:?}",
        st.enrichment
    );
}

...
fn a_jpeg_arrival_is_kept_as_original_and_worked_from_as_png() {
    ...
    assert!(
        r.media_path.starts_with("image/img/source/"),
        "{}",
        r.media_path
    );
    assert!(
        r.media_path.ends_with(&format!("{}.png", r.id)),
        "{}",
        r.media_path
    );
    let original = r
        .original_path
        .clone()
        .expect("a converted arrival keeps its original");
    ...
    let packet = pan::xmp::read_xmp_packet_from_bytes(&source)
        .unwrap()
        .expect("Pan's XMP is inside the source");
    ...
    assert_eq!(
        a.as_raw(),
        b.as_raw(),
        "same pixels in the PNG as the decoder saw"
    );
    ...
    assert_eq!(
        ...
        "the stored bytes are PNG, and the graph says so"
    );
    let source_file = facts
        .iter()
        .find(|(p, _)| p.ends_with("/sourceFile"))
        .map(|(_, v)| v[0].clone())
        .unwrap();
    assert_eq!(
        source_file, original,
        "pan:sourceFile names the original the PNG was made from"
    );
    assert!(
        packet.contains(&format!("<pan:sourceFile>{original}</pan:sourceFile>")),
        "sourceFile rides in the XMP"
    );
    let thumb = facts
        .iter()
        .find(|(p, _)| p.ends_with("/thumbnail"))
        .expect("a thumbnail node");
    assert_eq!(thumb.1.len(), 1);
    let thumb_path = std::fs::read_dir(store.layout.media_root.join("image/img/jpg"))
        .map(|_| ())
        .is_ok();
...
fn a_png_arrival_has_no_original() {
    ...
    let source_file = facts
        ...
        .unwrap();
    assert_eq!(
        source_file, r.media_path,
        "a PNG arrival's sourceFile is the source itself"
    );
    let thumbs: Vec<_> = walk(&store.layout.media_root.join("image/img/jpg"));
    assert_eq!(thumbs.len(), 1);
    assert!(
        thumbs[0].ends_with(&format!("{}_512.jpg", r.id)),
        "{}",
        thumbs[0]
    );
}
...
fn a_tiff_arrival_converts_to_png_with_the_same_pixels() {
    ...
    let original = r
        ...
        .expect("a converted arrival keeps its original");
    assert!(
        original.ends_with(&format!("{}.tiff", r.id))
            || original.ends_with(&format!("{}.tif", r.id)),
        "{original}"
    );
    let source = std::fs::read(store.layout.abs(&r.media_path)).unwrap();
    ...
    assert_eq!(
        a.as_raw(),
        b.as_raw(),
        "same pixels in the PNG as in the TIFF"
    );
}
...
fn the_thumbnail_struct_in_the_file_carries_the_produced_date_the_graph_has() {
    ...
    if let pan::QueryResults::Solutions(sols) = store.query(q).unwrap() {
        for s in sols {
            let s = s.unwrap();
            let pan::Term::Literal(l) = s.get("d").unwrap().clone() else {
                panic!("producedDate is a literal")
            };
    ...
    assert_eq!(
        graph_dates.len(),
        1,
        "one Thumbnail node with one producedDate: {graph_dates:?}"
    );
    assert_eq!(
        graph_dates[0], r.created_date,
        "the thumbnail is produced when the image is created"
    );
    ...
    assert_eq!(
        file_date.as_deref(),
        Some(graph_dates[0].as_str()),
        "pan:producedDate in the file's thumbnail struct equals the graph's: {thumb:?}"
    );
}

...
structs = {"thumbnail"}
bags = {"captionData", "poseData", "vectorData"}
counted_bags = {"regionData"}
# Multi-valued plain strings: an rdf:Bag of text values.
lists = {"sceneObjects"}

def tag_lines(names, skip=()):
    return "\n".join(f"    {n} => {{ List => 'Bag' }}," if n in lists else f"    {n} => {{ }}," for n in names if n not in skip)
...
%panRef = (
    ...
    STRUCT_NAME => 'PanRef',
    id => {{ }}, model => {{ }}, path => {{ }}, producedDate => {{ }},
);

%panRegionRef = (
    NAMESPACE   => {{ 'pan' => 'https://repolex.ai/ontology/pan/' }},
    STRUCT_NAME => 'PanRegionRef',
...
%Image::ExifTool::UserDefined::pan = (
    ...
    NAMESPACE => {{ 'pan' => 'https://repolex.ai/ontology/pan/' }},
    WRITABLE  => 'string',
{tag_lines(pan, structs | bags | counted_bags)}
    thumbnail => {{
        Struct => {{
            NAMESPACE   => {{ 'pan' => 'https://repolex.ai/ontology/pan/' }},
...
{chr(10).join(f"    {b} => {{ List => 'Bag', Struct => \\%panRegionRef }}," for b in sorted(counted_bags))}
);
